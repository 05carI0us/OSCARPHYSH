...
def add_cloudflare_prompt():
    ...
    print("\n{0}[{1}*{0}]{0}VEUX TU AJOUTER UNE FAUSSE PAGE DE PROTECTION CLOUDFARE ANTI DDOS POUR PLUS DE SÉCURITÉ ET DE PROFESSIONALISME ? -{1}(Y/N)".format(default_palette[0], default_palette[4]))
    choice = input("\n\n{0}VOTRE CHOIX >>> {1}".format(default_palette[0], default_palette[2])).upper()
    if choice == 'Y':
        add_cloudfare()
    else:
        wait(1)
